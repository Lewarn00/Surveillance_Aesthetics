...
def display_images():
    os.environ['SDL_VIDEO_WINDOW_POS'] = f"{1200},{0}"  # edit the x value here to control which display it is on
    windowSurface = pygame.display.set_mode((3840, 2160), pygame.NOFRAME)  # window spans two displays

    while True:
        # adjust these to match up with model output
        latest_images = get_latest_images('dolly_moun/generated_art/')
        portrait_img = pygame.image.load(latest_images[0])
        scene_img = pygame.image.load(latest_images[1])

        portrait_img = pygame.transform.scale(portrait_img, (1920, 1080))
        scene_img = pygame.transform.scale(scene_img, (1920, 1080))
