...
class Vgg16_Extractor(nn.Module):
    ...
    def forward(self, x):
        if self.space != 'vgg':
            x = (x + 1.) / 2.
            x = x - (torch.Tensor([0.485, 0.456, 0.406]).view(1, -1, 1, 1))
            x = x / (torch.Tensor([0.229, 0.224, 0.225]).view(1, -1, 1, 1))
...
def spatial_feature_extract(feat_result, feat_content, xx, xy):
    ...
    for i in range(len(feat_result)):
        ...
        w00 = torch.from_numpy((1.-xxr)*(1.-xyr)).float().view(1, 1, -1, 1)
        w01 = torch.from_numpy((1.-xxr)*xyr).float().view(1, 1, -1, 1)
        w10 = torch.from_numpy(xxr*(1.-xyr)).float().view(1, 1, -1, 1)
        w11 = torch.from_numpy(xxr*xyr).float().view(1, 1, -1, 1)
    ...
    xx = torch.from_numpy(xx).view(1,1,x_st.size(2),1).float()
    yy = torch.from_numpy(xy).view(1,1,x_st.size(2),1).float()
...
def rgb_to_yuv(rgb):
    C = torch.Tensor([[0.577350,0.577350,0.577350],[-0.577350,0.788675,-0.211325],[-0.577350,-0.211325,0.788675]])
    yuv = torch.mm(C,rgb)
    return yuv
...
def strotss(content_pil, style_pil, content_weight=1.0*16.0, device='cuda:0', space='uniform'):
    ...
    content_full = np_to_tensor(content_np, space)
    style_full = np_to_tensor(style_np, space)

    lr = 2e-3
    extractor = Vgg16_Extractor(space=space)
